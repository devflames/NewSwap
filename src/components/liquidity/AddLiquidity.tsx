...
const AddLiquidity = ({
  ...
  useEffect(() => {
    checkTokenAllowance(tokenOne.address).then((res) => {
      setIsAllowed(res);
    });
    checkTokenAllowance(tokenTwo.address).then((res) => {
      setIsAllowed(res);
    });
  }, [tokenOne, tokenTwo]);
  ...
  const approve = () => {
    approveTokens([tokenOne.address, tokenTwo.address]);
    setIsAllowed(true);
  };
  ...
  return (
    <section className="flex flex-col items-center  bg-primary text-secondary gap-4 px-5 py-3 pb-5 w-[27rem] min-h-[410px] rounded-3xl mb-6">
      ...
      <main className="flex flex-col gap-3 items-center w-full antialiased">
        ...
        />
        {isConnected ? (
          <section className="py-3 mt-3 border-rounded-xl text-start flex flex-col gap-3 w-full items-center">
      ...
      {isConnected && address ? (
        <Button
          ...
          callback={isAllowed ? () => handleAddLiquidity() : () => approve()}
          placeholder={`${
            isAllowed === false
              ? "Approve"
